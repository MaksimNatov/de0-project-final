...
def extract_from_postgres(**context):
    ...
    if transactions_data:
        insert_transactions = """
            INSERT INTO STV202506164__STAGING.transactions 
        ...
        """
        vertica_hook.insert_rows('STV202506164__STAGING.transactions', transactions_data)
    ...
    if currencies_data:
        insert_currencies = """
            INSERT INTO STV202506164__STAGING.currencies 
            (date_update, currency_code, currency_code_with, currency_with_div)
            VALUES (%s, %s, %s, %s)
        """
        vertica_hook.insert_rows('STV202506164__STAGING.currencies', currencies_data)
        logger.info(f"Currencies loaded: {len(currencies_data)}")
    else:
        logger.info("No currencies data to load")
